...
impl<'a> WorldUIContext<'a> {
    ...
    pub fn entity_ui(
        ...
        if show_entity {
            CollapsingHeader::new(entity_name)
    ...
    pub fn entity_ui_inner(
        ...
        changed |= self.component_kind_ui(
            ui,
            |archetype| archetype.components().collect(),
            "Components",
            entity,
            params,
            id,
        ...
        changed |= self.component_kind_ui(
            ui,
            |archetype| archetype.sparse_set_components().collect(),
            "Components (Sparse)",
            entity,
            params,
            id,
    ...
    pub fn component_kind_ui(
        &mut self,
        ui: &mut egui::Ui,
        components: impl Fn(&Archetype) -> Vec<ComponentId>,
        title: &str,
        entity: Entity,
        params: &WorldInspectorParams,
        id: egui::Id,
...
fn display_by_reflection(
    ...
    let mut reflected = {
        ...
        reflect_component.reflect_mut(world, entity).ok_or(())?
    };
    ...
    match component_info.storage_type() {
        ...
        StorageType::SparseSet => {
            world
                ...
                .get(component_id)
                .and_then(|sparse_set| {
                    // let data = sparse_set.get(entity).and_then(|s| Some(s.as_ptr()));
                    // let ticks = sparse_set.get_ticks(entity);

                    // data.zip(ticks).map(|(d, mut t)| {
                    //     let t: *mut _ = &mut t;
                    //     (d, t)
                    // })
                    sparse_set
